...
class PatternDetector:
    ...
    def _detect_volume_spike(self, ticker: str, company_name: str) -> Optional[PatternAlert]:
    ...
    def _detect_sentiment_shift(self, ticker: str, company_name: str) -> Optional[PatternAlert]:
        ...
        older_articles = self._get_company_articles(
            ticker, 
            hours=48, 
            exclude_hours=self.windows['medium']
        )
        
        if older_articles:
            older_sentiments = [self.sentiment_analyzer.analyze(a['content']) for a in older_articles]
    ...
    def _detect_momentum_building(self, ticker: str, company_name: str) -> Optional[PatternAlert]:
    ...
    def _detect_negative_cluster(self, ticker: str, company_name: str) -> Optional[PatternAlert]:
        ...
        for article in recent_articles:
            ...
            if sentiment < -0.3:
                ...
                for word in ['investigation', 'lawsuit', 'layoffs', 'bankruptcy', 
                            'crash', 'plunge', 'scandal', 'fraud']:
