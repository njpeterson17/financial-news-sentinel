...
class AlertManager:
    ...
    def __init__(self, config: Dict[str, Any], db: Database):
        ...
        self.telegram_token = os.getenv('NEWS_BOT_TELEGRAM_TOKEN', 
                                       config.get('telegram', {}).get('bot_token', ''))
        self.telegram_chat_id = os.getenv('NEWS_BOT_TELEGRAM_CHAT_ID',
                                         config.get('telegram', {}).get('chat_id', ''))
        
        # Webhook config
        self.webhook_enabled = config.get('webhook', {}).get('enabled', False)
        self.webhook_url = os.getenv('NEWS_BOT_WEBHOOK_URL',
    ...
    def _retry_with_backoff(
        ...
        for attempt in range(max_retries):
            try:
                ...
                last_exception = e
                logger.warning(
                    f"{description} attempt {attempt + 1}/{max_retries} timed out: {e}"
                )
            except ConnectionError as e:
                last_exception = e
                logger.warning(
                    f"{description} attempt {attempt + 1}/{max_retries} connection failed: {e}"
                )
                ...
                if e.response is not None and 400 <= e.response.status_code < 500:
                    if e.response.status_code != 429:
                        logger.error(
                            f"{description} failed with client error {e.response.status_code}: {e}"
                        )
                        return False
                logger.warning(
                    f"{description} attempt {attempt + 1}/{max_retries} failed with HTTP error: {e}"
                )
            except RequestException as e:
                last_exception = e
                logger.warning(
                    f"{description} attempt {attempt + 1}/{max_retries} failed: {e}"
                )

            # Calculate delay with exponential backoff
            if attempt < max_retries - 1:
                delay = initial_delay * (2 ** attempt)
                logger.debug(f"Retrying {description} in {delay:.1f}s...")
                time.sleep(delay)
    ...
    def _telegram_alert(self, alert: PatternAlert):
        ...
        success = self._retry_with_backoff(
            send_request,
            description=f"Telegram alert for {alert.ticker}"
        )
    ...
    def _webhook_alert(self, alert: PatternAlert):
        ...
        success = self._retry_with_backoff(
            send_request,
            description=f"Webhook alert for {alert.ticker}"
        )

        if success:
            logger.info(f"Webhook alert sent for {alert.ticker}")
