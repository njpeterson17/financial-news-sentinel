...
try:
    ...
    logger.warning("vaderSentiment not installed. ML sentiment analysis unavailable. "
...
class CompanyExtractor:
    ...
    def _build_patterns(self):
        ...
        self.ticker_pattern = re.compile(
            r'\$([A-Z]{1,5})\b|\b([A-Z]{2,5})\s+(?:stock|shares|equity)',
            re.IGNORECASE
        )
    ...
    def _calculate_confidence(self, name: str, context: str) -> float:
        ...
        financial_words = ['stock', 'shares', 'trading', 'investors', 'earnings', 
                          'revenue', 'profit', 'market', 'company', 'corp', 'inc']
...
class KeywordSentimentAnalyzer:
    ...
    def __init__(self, positive_words: List[str], negative_words: List[str]):
        ...
        self.positive_pattern = re.compile(
            r'\b(' + '|'.join(re.escape(w) for w in self.positive_words) + r')\b',
            re.IGNORECASE
        )
        self.negative_pattern = re.compile(
            r'\b(' + '|'.join(re.escape(w) for w in self.negative_words) + r')\b',
            re.IGNORECASE
        )

        # Intensity modifiers
        self.intensifiers = ['very', 'extremely', 'significantly', 'massively',
                            'hugely', 'dramatically', 'substantially']
...
class SentimentAnalyzer:
    ...
    def __init__(
        ...
        self._keyword_analyzer: Optional[KeywordSentimentAnalyzer] = None
        self._ml_analyzer: Optional[MLSentimentAnalyzer] = None
    ...
    def _init_analyzers(self):
        ...
        if self.method == SentimentMethod.KEYWORD:
            if not self._positive_words and not self._negative_words:
                raise ValueError(
                    "Keyword method requires positive_words and/or negative_words"
                )
